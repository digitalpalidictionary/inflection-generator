...
open("output/sandhi/sandhi2 matches.txt", "w")
...
def import_text_set():
	...
	# unmatched_df = pd.read_csv("output/sandhi/unmatched.csv", dtype=str, header = None)
	# text_set = set(unmatched_df[0].tolist())
...
# 	print(f"{timeis()} {green}sandhi splitting")
...
# 		wordB = (word[-x-1:])
# 		wordB_firstletter = (wordB[:1])
# 		# print(wordA, wordB)
# 		# print(f"{timeis()} {restword} {lastword}")
# 		# fuzzy = re.compile(f"^.{lastword[1:]}$")
# 		# match = list(filter(fuzzy.match, all_inflections_set))
# 		# print(f"{timeis()} {match}")
# 		if wordB in all_inflections_set:
# 			# print(wordA, wordB)
# 			matches_list = []
# 			matches_list.append(wordB)
# 			# word = word.removesuffix(wordB)
# 			# print(word)
# 		if wordB in all_inflections_set:
# 			for rule in rules:
...
def recursion(word):
	...
	# print(f"{blue}{word}")
	first_part = re.sub(" .*$", "", word)
	last_part = re.sub("^.?[^ ]+", "", word)
	last_part_firstletter = last_part[:-1]
	print(f"{blue}{first_part} {last_part}")
	word_first_length = len(first_part)
	if len(first_part) > 0:
		for x in range(word_first_length):
			wordA = (first_part[:x])
			wordB = (first_part[x:])
			wordA_lastletter = wordA[-1:]
			wordB_firstletter = (wordB[:1])
			print(f"{white}\t{x} {wordA}-{wordB} {green}{last_part}")
			if wordB in all_inflections_set:
					# print(f"{green}\t{wordB}")
					match_list.append(wordB)
					# word = word.removesuffix(wordB)
					word = wordA + " " + wordB + last_part
					# print(f"{red}\t{word}")
					recursion(word)
					return(word)
			else:
				for rule in rules:
					...
					wordA_lastletter == chA and \
					wordB_firstletter == chB:
						word1 = first_part[:-x-2] + ch1
						word2 = ch2 + first_part[-x:]
						
						if word2 in all_inflections_set:
							print(f"\t{white}{word1} {yellow}{word2} {green}{last_part}")
							match_list.append(word2)
							word = word1 + " " + word2 + last_part
							print(f"{red}\t{word}")
							recursion(word)
							return(word)
	...
	if len(first_part) == 0:
		match_list.reverse()
		print(f"{yellow}{match_list}\n")
		with open("output/sandhi/sandhi2 matches.txt", "a") as f:
			f.write(f"{word} {match_list}\n")
		# print(f"{yellow}{word}\n")
...
import_sandhi_rules()
remove_exceptions()
# sandhi_splitter("pañcavidhabandhanādikammakaraṇatthaṃ")
# recursion("dhammakaraṇakammaahaṃ")
sandhi_splitter()
# two_word_sandhi()
# three_word_sandhi()
# four_word_sandhi()
